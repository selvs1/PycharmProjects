...
class Unary_expression(Term):
    ...
    def __init__(self, term: Term, una_op: str) -> None:
        ...
        :param una_op: binary operator (string, "-", etc.)
        """
        self.term = term
        self.una_op = una_op
...
def main() -> None:
    ...
    neg = Unary_expression(v, "-")
    print("-d: ", neg.eval(ctx))
    addition = Binary_expression(three, neg, "+")
    print("3 + -d: ", addition.eval(ctx))
    multiplication = Binary_expression(addition, five, "*")
